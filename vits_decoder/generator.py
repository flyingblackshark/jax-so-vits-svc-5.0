...
class Generator(nn.Module):
    ...
    def setup(self):
        ...
        self.adapter = SpeakerAdapter(self.hp.vits.spk_dim, self.hp.gen.upsample_input)
        # pre conv
        self.conv_pre = nn.Conv(features=self.hp.gen.upsample_initial_channel, kernel_size=[7], strides=[1],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
        # nsf
        ...
        for i, (u, k) in enumerate(zip(self.hp.gen.upsample_rates, self.hp.gen.upsample_kernel_sizes)):
            ...
            ups.append(
                    nn.ConvTranspose(
                        ...
                        (k,),
                        (u,),kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
                )
            # nsf
            if i + 1 < len(self.hp.gen.upsample_rates):
                ...
                noise_convs.append(
                    nn.Conv(
                        ...
                        strides=[stride_f0],
                        dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01)
                    )
                )
            else:
                noise_convs.append(
                    nn.Conv(features=self.hp.gen.upsample_initial_channel //
                           (2 ** (i + 1)), kernel_size=[1],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
                )
        ...
        # post conv
        self.conv_post = nn.Conv(features=1, kernel_size=[7], strides=1 , use_bias=False,dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
