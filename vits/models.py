...
class TextEncoder(nn.Module):
    ...
    p_dropout:float
    def setup(self):
        self.pre = nn.Conv(features=self.hidden_channels, kernel_size=[5],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
        ...
        self.proj = nn.Conv(features=self.out_channels * 2, kernel_size=[1],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
...
class PosteriorEncoder(nn.Module):
    ...
    def setup(
        ...
        self.pre = nn.Conv(features=self.hidden_channels, kernel_size=[1],kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
        self.enc = modules.WN(
            self.hidden_channels,
            self.kernel_size,
            self.dilation_rate,
        ...
        self.proj = nn.Conv(features=self.out_channels * 2,kernel_size=[1],kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
