...
class FFN(nn.Module):
    ...
    def setup(self):
        self.conv_1 = nn.Conv(self.filter_channels, [self.kernel_size],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
        self.conv_2 = nn.Conv(self.out_channels, [self.kernel_size],dtype=jnp.float32,kernel_init=nn.initializers.normal(),bias_init=nn.initializers.normal(0.01))
        self.drop = nn.Dropout(self.p_dropout)
